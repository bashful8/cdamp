...
func newTestAdminMux(t *testing.T) (http.Handler, *fakes.InboxStoreFake, *fakes.AdminStoreFake, *config.Config) {
	t.Helper()
	...
	return NewAdminMux(inbox, admin, cfg), inbox, admin, cfg
}
...
func TestAdminMuxCreateAgentHappyPath(t *testing.T) {
	mux, inbox, _, _ := newTestAdminMux(t)
...
func TestAdminMuxNoCredentialBootstrappedRejected(t *testing.T) {
	...
	cfg := &config.Config{Domain: "example.dev"}
	mux := NewAdminMux(inbox, admin, cfg)
